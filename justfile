...
release-notes:
    ...
    echo "### Security"
    echo ""
    echo "**Full Changelog**: https://github.com/hongkongkiwi/elevenlabs-cli/compare/$(git describe --tags --abbrev=0 2>/dev/null || echo "previous")...v$(grep '^version = ' Cargo.toml | head -1 | sed 's/version = "\(.*\)"/\1/')"

# Local dry-run release build
release-local:
    #!/usr/bin/env bash
    echo "Building release artifacts locally..."
    cargo build --release
    echo ""
    echo "Binary built at: target/release/elevenlabs-cli"
...
install-github VERSION="":
    ...
    echo "Installing elevenlabs-cli v$VERSION from source..."
    cargo install --git https://github.com/hongkongkiwi/elevenlabs-cli --tag "v$VERSION" --locked
...
help:
    @echo "ElevenLabs CLI - Available Commands"
    @echo ""
    @echo "Build:"
    @echo "  build              - Build debug binary"
    @echo "  build-release      - Build release binary"
    @echo "  build-mcp          - Build with MCP support"
    @echo "  build-all-features - Build with all features"
    @echo "  build-target       - Build for specific target"
    @echo ""
    @echo "Development:"
    @echo "  dev                - Run CLI with args"
    @echo "  dev-mcp            - Run MCP server mode"
    ...
    @echo "  release-cargo      - Release using cargo-release"
    @echo "  release-dry-run    - Dry-run cargo-release"
    @echo "  release [type]     - Manual release (patch/minor/major)"
